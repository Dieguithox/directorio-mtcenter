...
$nombre = $_SESSION['usuario']['nombre'] ?? 'Editor';
?>
...
<head>
    ...
    <!-- CSS -->
    <link rel="stylesheet" href="<?= BASE_URL ?>/View/contenido/css/styleEditor.css">
    <!-- Font Awesome -->
...
<aside id="sidebar" class="sidebar" aria-label="Menú lateral">
    ...
    <div class="nav-group">
        ...
        <li class="nav-item">
            ...
            href="<?= BASE_URL ?>/config/app.php?accion=editor.reportes">
            <i class="bi bi-graph-up"></i><span>Reportes</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link <?= ($active ?? '')==='cambios'?'active':'' ?>"
            href="<?= BASE_URL ?>/config/app.php?accion=editor.solicitudes.cambio">
...
<main class="main">
    <div class="container-fluid">
        <div class="mb-3"><span class="badge rounded-pill badge-role">Editor · Vista principal</span></div>
        <!-- CONTENIDO -->

<!-- Drawer de notificaciones (el mismo que admin) -->
...
<script>
    ...
    // ====== Notificaciones (mismo JS que admin, solo cambié el BASE_URL) ======
    (function() {
        ...
        function renderItems(items) {
            if (!items.length) return '<div class="text-center text-muted py-4">Sin notificaciones</div>';
            return items.map(n => {
